...
One code path used by BOTH the `model.predict` CLI and the FastAPI endpoint
(Phase 4b), so there is no drift between how predictions are made on the
command line and over HTTP.

Also implements artifact hot-reload: the weekly ETL overwrites the files in
models/ while a server may be running. Before each prediction we compare the
modification time of models/metrics.json (always rewritten last by train.py)
against the loaded bundle; if it changed, we transparently reload the model,
calibrator, and feature metadata. The operator never needs to restart the
API after a weekly ETL run.
"""
