"""Polite HTTP client for nrl.com and Open-Meteo."""
...
class RateLimitedHttpClient:
    def __init__(self, delay_seconds: float = 1.0, max_retries: int = 3, timeout: int = 30):
    ...
    def absolute_nrl(self, url: str) -> str:
        if url.startswith("/"):
            return NRL_BASE + url
        return url

    def get_text(self, url: str, *, headers: dict | None = None) -> str:
        url = self.absolute_nrl(url)
